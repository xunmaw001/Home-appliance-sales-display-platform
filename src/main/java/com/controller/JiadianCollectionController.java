...
public class JiadianCollectionController {
    private static final Logger logger = LoggerFactory.getLogger(JiadianCollectionController.class);

    private static final String TABLE_NAME = "jiadianCollection";
    ...
    */
    ...
    public R save(@RequestBody JiadianCollectionEntity jiadianCollection, HttpServletRequest request){
        logger.debug("save方法:,,Controller:{},,jiadianCollection:{}",this.getClass().getName(),jiadianCollection.toString());
    ...
    */
    ...
    public R update(@RequestBody JiadianCollectionEntity jiadianCollection, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,jiadianCollection:{}",this.getClass().getName(),jiadianCollection.toString());
        JiadianCollectionEntity oldJiadianCollectionEntity = jiadianCollectionService.selectById(jiadianCollection.getId());//查询原先数据
...
//            jiadianCollection.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<JiadianCollectionEntity> oldJiadianCollectionList =jiadianCollectionService.selectBatchIds(Arrays.asList(ids));//要删除的数据
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<JiadianCollectionEntity> jiadianCollectionList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            JiadianCollectionEntity jiadianCollectionEntity = new JiadianCollectionEntity();
//                            jiadianCollectionEntity.setJiadianId(Integer.valueOf(data.get(0)));   //商品 要改的
//                            jiadianCollectionEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            jiadianCollectionEntity.setJiadianCollectionTypes(Integer.valueOf(data.get(0)));   //类型 要改的
//                            jiadianCollectionEntity.setInsertTime(date);//时间
//                            jiadianCollectionEntity.setCreateTime(date);//时间
                            jiadianCollectionList.add(jiadianCollectionEntity);
                        ...
                        //查询是否重复
                        jiadianCollectionService.insertBatch(jiadianCollectionList);
                        return R.ok();
                    }
                }
            }
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody JiadianCollectionEntity jiadianCollection, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,jiadianCollection:{}",this.getClass().getName(),jiadianCollection.toString());
        Wrapper<JiadianCollectionEntity> queryWrapper = new EntityWrapper<JiadianCollectionEntity>()
            .eq("jiadian_id", jiadianCollection.getJiadianId())
            .eq("yonghu_id", jiadianCollection.getYonghuId())
            .eq("jiadian_collection_types", jiadianCollection.getJiadianCollectionTypes())
//            .notIn("jiadian_collection_types", new Integer[]{102})
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        JiadianCollectionEntity jiadianCollectionEntity = jiadianCollectionService.selectOne(queryWrapper);
        if(jiadianCollectionEntity==null){
